...
void page_init(page_t *o, void *begin, unsigned long length,
  ...
  for (int i = 0; i < freemap_pages; i++)
    page_alloc(o);
  unsigned long mask = ((unsigned long)1 << (o->pages_total % CELL_BITS)) - 1;
  o->freemap[o->freemap_cells - 1] = o->freemap[o->freemap_cells - 1] & mask;
}
  ...
  for (unsigned long i = 0; i < o->freemap_cells; i++) {
    if (o->freemap[i] != 0) {
      for (unsigned long j = 0; j < CELL_BITS; j++) {
        unsigned long cellmask = ((unsigned int)1 << j);
        if (o->freemap[i] & cellmask) {
          o->freemap[i] &= ~cellmask;
          unsigned long pagenumber = i * CELL_BITS + j;
          void *pageaddr = (pagenumber << o->page_bits) + (char*)o->freemap;
          o->pages_free--;
          return pageaddr;
        }
      }
